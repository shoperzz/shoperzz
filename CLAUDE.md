...
shoperzz/
├── apps/
│   ├── api/            ← NestJS business API (user auth, subscriptions, newsletter)
│   ├── web/            ← shoperzz.dev marketing site
│   ├── docs/           ← documentation site (Nextra / Docusaurus)
│   └── admin/          ← platform admin UI (manage users, plugins, billing)
├── packages/
...
├── cli/                ← @shoperzz/cli
├── demos/              ← non-published e-commerce reference projects
...
- **Unit tests** → `__tests__/` inside each package (e.g., `plugins/payment-orange-money/__tests__/`) — mock all external deps
- **Integration tests** → `__tests__/` inside each package, use `createTestApp()` from `@shoperzz/testing` with SQLite in-memory
- **E2E tests** → `e2e/` at repo root — cross-package only (e.g., full checkout flow spanning core + payment plugin)
